...
\\
View the full list of webhook events.](https://platform.openai.com/docs/api-reference/webhook-events)

Below are examples of simple servers capable of ingesting webhooks from OpenAI, specifically for the [`response.completed`](https://platform.openai.com/docs/api-reference/webhook-events/response/completed) event.
...
To see a webhook like this one in action, you can set up a webhook endpoint in the OpenAI dashboard subscribed to `response.completed`, and then make an API request to [generate a response in background mode](https://platform.openai.com/docs/guides/background).
